...
**2. ChromaDB + sentence-transformers for all vector operations.**  
Do NOT use OpenAI embeddings (costs money per call). Use `all-MiniLM-L6-v2` from HuggingFace sentence-transformers — it runs locally with zero API cost. Do NOT use Pinecone or any hosted vector DB for V1 — ChromaDB persists to disk locally.
...
**7. No user authentication for V1.**  
No login, no JWT, no user accounts. All data belongs to the single instance. Keep it simple.
...
**Embedding at startup:** The sentence-transformers model takes 5–15 seconds to load on first import. Load it once at module level in `vector_service.py`, not inside each function call. FastAPI startup will be slightly slow but subsequent calls will be fast.
...
DATABASE_URL=sqlite:///./routeeasy.db
