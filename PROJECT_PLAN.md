...
- [ ] **T001** — Create monorepo: `routeeasy/frontend/` and `routeeasy/backend/`
...
- [ ] **T019** — Load `all-MiniLM-L6-v2` at module level (runs locally, no API): `SentenceTransformer("all-MiniLM-L6-v2")`
...
- [ ] **T124** — Containerise backend with `Dockerfile` (Python 3.11 slim, copy app, install deps, run uvicorn)
- [ ] **T125** — Deploy backend to GCP Cloud Run or Railway — set all env vars in platform dashboard
- [ ] **T126** — Build frontend: `npm run build` → deploy `dist/` to Vercel
- [ ] **T127** — Set `VITE_API_BASE_URL` in Vercel to production backend URL
- [ ] **T128** — Set `CORS_ORIGINS` in backend to Vercel deployment URL
- [ ] **T129** — Test full production flow: chat → agent runs → stops previewed → trip saved → semantic search finds it → RAG query answers question about it
- [ ] **T130** — Share URL — give to your father and use as portfolio showcase link
...
Phase 0 (Setup)
→ Phase 1 (Databases)
→ Phase 2 (Embeddings)          ← HuggingFace sentence-transformers
...
- Full build (Phases 0–17): 1.5–2 weeks
