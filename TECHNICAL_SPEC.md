...
| **Embeddings** | sentence-transformers (`all-MiniLM-L6-v2`) | Latest | HuggingFace, runs locally, no API cost |
| **Vector DB** | ChromaDB | 0.5+ | Local-first, persistent, easy to deploy |
| **Relational DB** | SQLite via SQLAlchemy | Latest | Zero-config, file-based |
...
| **Deployment** | GCP Cloud Run (backend) + Vercel (frontend) | — | Free tiers, container-based |
...
│  │  (trips,     │  │  (stop + trip  │  │  all-MiniLM-L6-v2   │  │
│  │  stops,      │  │   embeddings,  │  │  (local embeddings)  │  │
│  │  history,    │  │   history      │  │                      │  │
│  │  llm_logs)   │  │   embeddings)  │  │                      │  │
│  └──────────────┘  └────────────────┘  └──────────────────────┘  │
│                                                                    │
...
### SQLite Tables (Relational / Structured Data)
...
# Load embedding model once at startup (runs locally, no API calls)
...
| `POST` | `/trips/{id}/launch` | Record launch, update stats, add to history ChromaDB |
...
| Route | Component |
|-------|-----------|
| `/` | `HomeScreen` — quick-launch saved trips grid |
...
| `/admin/logs` | `LLMLogsScreen` — token usage dashboard (showcase only) |
