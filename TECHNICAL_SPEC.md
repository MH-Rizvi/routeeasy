...
| **Embeddings** | fastembed (`BAAI/bge-small-en-v1.5`) | Latest | Lightweight ONNX, natively embedded directly across Railway for compliance |
...
│                                                                    │
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │              History Q&A Pipeline                            │  │
│  │  Question → SQL retrieval (trips/history) → Fallback LLM     │  │
│  └─────────────────────────────────────────────────────────────┘  │
│                                                                    │
│  ┌──────────────┐  ┌──────────────────────┐                    │  │
│  │  Supabase    │  │  fastembed           │                    │  │
│  │ PostgreSQL + │  │  BAAI/bge-small-en  │                    │  │
...
### Vector Storage Architecture (Compliance)

The only vector storage used in the application is for the CDL Compliance Assistant. We utilize `pgvector` inside our Supabase PostgreSQL instance (`compliance_chunks` table).

**Note:** Trip and stop searches use high-performance SQL `ILIKE` pattern matching in PostgreSQL, ensuring 100% search reliability in ephemeral cloud environments. History Q&A has also been migrated to pure SQL database queries (`trip_history` and `trips` tables) replacing the previous semantic vector implementations entirely.
...
## 5. History Q&A Pipeline Implementation Flow (Pure SQL)

1. The user asks "Have I been to Oak Street?".
2. The `rag_service.py` securely extracts the user's `trip_history` and `trips` via standard SQLAlchemy relational queries.
3. It derives aggregated driving statistics (e.g., total historical miles, total trip count) natively.
4. It converts the retrieved payload into generic Context parameters injected directly inside a restricted Prompt instructing the Groq/Gemini array to answer based only on provided Context.
5. The User receives a formatted markdown answer referencing retrieved structured data directly.
